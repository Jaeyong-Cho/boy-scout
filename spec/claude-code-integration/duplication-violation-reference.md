...
- **`internal/setup/SKILL.md`'s "Fix one violation per violation-type per run" paragraph is updated:** fix one violation from each violation type in each run (e.g., one funclen + one crap + one filelen = 3 violations in one run); within each type, fix the worst violation first (ranked by severity: lines-over-limit for funclen/filelen, CRAP score for crap, DupLines for duplication, Gap for instability, Distance for abstractness).
...
|Given `SKILL.md`'s cap paragraph - When read - Then it states fix one violation per violation-type per run, with examples showing multiple types can be fixed in one run (e.g., funclen + crap + filelen)|Normal|unit test: `internal/setup/setup_test.go: TestRun_TemplateStatesClusterCountsAsOnePerType`|
